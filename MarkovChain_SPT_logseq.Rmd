...
#test_df2 <- as.numeric(gsub("1", 1, gsub("0", 0, test_df2)))
tidydf <- test_df %>% gather(SIM, ACT, V1:V20)

test_df3 <- test_df2 %>% bind_rows(summarise_all(., ~if(is.numeric(.)) sum(.) else "Total"))

#test <- by(test_df, )
#tidydf$act.count <- ave(tidydf$ACT ==1, tidydf$SIM, FUN=cumsum)

#test_df$act.count <- ave(test_df[1:20] == 1, test_df$days, FUN=cumsum)  
  
  
#mcDf = data.frame()

#for (i in length(1:532)) {
#  tmp_df <- subset(test_df, days == i)
