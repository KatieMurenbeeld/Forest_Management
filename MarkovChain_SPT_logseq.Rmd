...
df_sid <- df_sid %>% 
  mutate(ACT_TYPE2 = case_when(
    ACTIVITY_TYPE == 'NO_ACT' ~ '0',
    ACTIVITY_TYPE == 'INT_CUT' ~ '1',
    ACTIVITY_TYPE != 'NO_ACT' & ACTIVITY_TYPE != 'INT_CUT' ~ '2'
  ))
#df_sid$ACT_TYPE2 <- as.character(df_sid$ACT_TYPE2)
...
#df_reg$ACT_TYPE2 <- as.character(df_reg$ACT_TYPE2)
...
# New dataset with more Nat. Forests added. Very simplified activity types and has binary Action - No Action?
seqmat_reg3 <- createSequenceMatrix(df_reg$ACT_TYPE2, toRowProbs = TRUE, sanitize=TRUE)
seqmat_reg3
...
seqmat_sid3 <- createSequenceMatrix(df_sid$ACT_TYPE2, toRowProbs = TRUE, sanitize=TRUE)
seqmat_sid3
...
markov_reg3 <- new('markovchain', 
               transitionMatrix = seqmat_reg3,
               states = seq_reg3)
...
markov_sid3 <- new('markovchain', 
               transitionMatrix = seqmat_sid3,
               states = seq_sid3)
...
reg3_sims_mu <- replicate(100, {
  sequence <- rmarkovchain(n = 532, object = markov_reg3, t0 = "0")
})
...
sid3_sims_mu <- replicate(100, {
  sequence <- rmarkovchain(n = 543, object = markov_sid3, t0 = "0")
})
```
...
reg3_sims_med <- replicate(20, {
  sequence <- rmarkovchain(n = 149, object = markov_reg3, t0 = "0")
})
...
sid3_sims_med <- replicate(20, {
  sequence <- rmarkovchain(n = 190, object = markov_sid3, t0 = "0")
})
```
...
reg3_sims_mu2 <- replicate(100, {
  sequence <- rmarkovchain(n = 532, object = markov_reg3, t0 = "1")
})

reg3_sims_mu3 <- replicate(1000, {
  sequence <- rmarkovchain(n = 532, object = markov_reg3, t0 = "1")
})
...
sid3_sims_mu2 <- replicate(100, {
  sequence <- rmarkovchain(n = 543, object = markov_sid3, t0 = "1")
})

sid3_sims_mu3 <- replicate(1000, {
  sequence <- rmarkovchain(n = 543, object = markov_sid3, t0 = "1")
})
```
...
```{r sid3_1000, echo=TRUE, eval=TRUE, fig.height=6, fig.width=15, fig.align='center'}
# For S. Idaho. 1000 synthetic sequences

#day.count <- c(1:543)
df_sid3_mu3 <- as.data.frame(sid3_sims_mu3)
df_sid3_mu3$days <- c(1:543)

df_sid3_mu3 <- df_sid3_mu3 %>% gather(key = SIM, value = ACT_TYPE2, V1:V100)
df_sid3_mu3$ACT_TYPE2 <- ifelse(df_sid3_mu3$ACT_TYPE2 =='1', 1, ifelse(df_sid3_mu3$ACT_TYPE2 =='2', 2, 0))

sid3_days_act1 <- filter(df_sid3_mu3, ACT_TYPE2 == 1)
sid3_days_act2 <- filter(df_sid3_mu3, ACT_TYPE2 == 2) 
x1 = sid3_days_act1$days
h1 <- hist(sid3_days_act1$days, breaks = 100, plot=FALSE)
plot(h1, col="green")
x2 = sid3_days_act2$days
h2 <- hist(sid3_days_act2$days, breaks = 100, plot=FALSE)
plot(h2, col="red")
...
ggplot(sid3_day2_act1, aes(x=days)) + geom_histogram(binwidth = 1) + scale_x_continuous(breaks = seq(1, 543, 5)) + theme(axis.text.x = element_text(angle = 90))
```


```{r reg3_1000, echo=TRUE, eval=TRUE}
# For Region. 1000 synthetic sequences

#day.count <- c(1:532)
df_reg3_mu3 <- as.data.frame(reg3_sims_mu3)
df_reg3_mu3$days <- c(1:532)

df_reg3_mu3 <- df_reg3_mu3 %>% gather(key = SIM, value = ACT_TYPE2, V1:V100)
df_reg3_mu3$ACT_TYPE2 <- ifelse(df_reg3_mu3$ACT_TYPE2 =='1', 1, ifelse(df_reg3_mu3$ACT_TYPE2 =='2', 2, 0))

reg3_days_act1 <- filter(df_reg3_mu3, ACT_TYPE2 == 1)
reg3_days_act2 <- filter(df_reg3_mu3, ACT_TYPE2 == 2) 
x1 = reg3_days_act1$days
h1 <- hist(reg3_days_act1$days, breaks = 100, plot=FALSE)
plot(h1, col="green")
x2 = reg3_days_act2$days
h2 <- hist(reg3_days_act2$days, breaks = 100, plot=FALSE)
plot(h2, col="red")
...
ggplot(reg3_days_act1, aes(x=days)) + geom_histogram(binwidth = 1) + scale_x_continuous(breaks = seq(1, 543, 5)) + theme(axis.text.x = element_text(angle = 90))
```
